...
### v2.0（当前版本）
- 🔧 修复 polls.js 语法错误（页面崩溃）
- 🛡️ 修复 XSS 漏洞（所有用户输入转义）
- ⚡ 优化字体加载（@import 改为 link 预加载）
- 🧹 修复 Supabase 内存泄漏（页面卸载时清理订阅）
- 🧹 修复事件监听器泄漏（schedule.js）
- 🎨 修复你画我猜频繁上传（500ms 节流 + JPEG 压缩）
...
- 🎭 熊猫头像弹跳动画
- 💫 在线状态呼吸灯效果
- 🖼️ 64 个头像可选（动物、游戏、表情、太空等）
- 📝 更大字体、更高对比度
- 📅 时间表单元格优化（左上人数、左下✓、右边名字）
- 📜 可滑动列表（成员、通知）
- ✨ 按钮 hover 上浮效果
